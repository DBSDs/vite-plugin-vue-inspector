...
function VitePluginInspector(
  ...
  return [
      ...
      async load(id) {
        if (id === "virtual:vue-inspector-options") {
          ...
          if (fs.existsSync(file))
            ...
            console.error(
              `failed to find file for vue-inspector: ${file}, referenced by id ${id}.`
            );
        }
      },
      transform(code, id) {
        ...
        if (isJsx || isTpl)
          return compileSFCTemplate({
            ...
            disabledLineAndColumn: disabledLineAndColumn,
          });
      ...
      configureServer(server) {
        ...
        toggleComboKey &&
          (server.printUrls = () => {
            ...
            _printUrls();
            console.log(
              `  ${green("➜")}  ${bold("Vue Inspector")}: ${green(
    ...
    },
    {
      ...
      transform(code) {
        if (code.includes("_interopVNode")) return;
        if (!code.includes("data-v-inspector")) return;

        const fn = new Set<string>();
        const s = new MagicString(code);

        s.replace(
          /(createElementVNode|createVNode|createElementBlock|createBlock) as _\1,?/g,
          (_, name) => {
            fn.add(name);
            return "";
          }
